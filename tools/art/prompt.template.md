...
Generate a {pose} {com_name} ({sci_name}) in the style of an Edo-period Japanese kachō-e woodblock print, matching the painting technique of IMAGE 2 closely. Look at IMAGE 2: the bird is rendered with VERY FEW MARKS. The body is essentially 2-4 flat color zones with sharp boundaries. No pen-line stippling, no gradient shading, no soft airbrushed texture anywhere. Feather structure appears only where the real bird plainly shows it, and only on the wing coverts and scapulars - never scattered over the whole body. Look at IMAGE 2: most of that bird is flat colour, and the drawn feathers are confined to one area of the wing. YOUR output should look the same: a few flat color zones, a few confident outline strokes, an accent stroke or two for major wing or tail markings, and that's it.
...
- EXACTLY ONE BIRD. A single bird alone on the paper. No second bird, no mate, no partly-hidden bird behind the first, no repeated head or tail peeking out from anywhere. If you find yourself composing a pair, draw one and delete the other.
...
- PERCHED (pose 1): BOTH WINGS CLOSED AND FOLDED FLAT against the body, the far wing hidden behind the near one. The wings are NOT spread, NOT raised, NOT opening — this is a bird standing still. Both feet visible at the bottom, toes curled gently forward as if grasping a thin perch, but the perch itself is NOT drawn. The bird floats in space, posture suggesting it's perched. If the wings are open at all, this is the wrong pose.
- IN FLIGHT (pose 2): ONE bird alone in the air — not a pair, not a flock, and not one bird with a second pair of wings or a second tail showing behind it. A Galah and an Australian Raven have both come back as two overlapping birds in this pose. Both wings fully extended in a natural flapping position. Legs and feet either (a) tucked tight against the belly with toes folded out of sight, or (b) extended straight back along the line of the tail. Do NOT dangle the feet below the body with toes splayed.
