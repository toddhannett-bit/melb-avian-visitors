...
def build_tables(illus_dir: Path, only=None):
    ...
    for p in pngs:
        ...
        w, h = im.size
        # An illustration that still has its cream ground has no transparency,
        # so its "silhouette" is the whole rectangle. The collage packs by
        # silhouette, so that reserves a bird-sized box of blank paper and the
        # layout falls apart — and nothing about the output says so. This ran
        # to completion once on ten un-cut birds and reported success.
        if im.getchannel("A").getextrema()[0] > ALPHA_ON:
            uncut.append(slug)
            continue
    ...
    if uncut:
        raise SystemExit(
            f"error: {len(uncut)} illustration(s) have no transparent ground, so "
            f"their silhouette would be the whole rectangle:\n"
            + "\n".join(f"         {s}" for s in uncut[:12])
            + ("\n         …" if len(uncut) > 12 else "")
            + "\n\n       Run the cutout step first:\n"
              "         npm run art:cutout\n"
              "       Nothing has been written; dims.json and masks.json are "
              "unchanged.")
    return dims, masks
...
def main() -> int:
    ...
    if args.add:
        ...
        if missing:
            ...
            return 1
        cur_d = json.loads(dims_path.read_text(encoding='utf-8')) if dims_path.exists() else {}
        cur_m = json.loads(masks_path.read_text(encoding='utf-8')) if masks_path.exists() else {}
        cur_d.update(dims)
        cur_m.update(masks)
        dims_path.write_text(dump_perkey(cur_d), encoding='utf-8')
        masks_path.write_text(dump_perkey(cur_m), encoding='utf-8')
    ...
    dims_path.write_text(dump_perkey(dims), encoding='utf-8')
    masks_path.write_text(dump_perkey(masks), encoding='utf-8')
