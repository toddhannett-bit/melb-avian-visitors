(function () {
  ...
  var SKETCH_VERSION = 'r16'; // r16: Melbourne expansion — ten species added, plus Little Raven, Galah, Yellow-faced Honeyeater and Eurasian Coot redrawn. // r12: 84 eastern NA birds (PR #23) refined + re-cut. r11: full library restyle: every species
  ...
  var IMG_VERSION = 'r16'; // r16: as SKETCH_VERSION. // r12: 84 eastern NA birds (PR #23) refined + re-cut. r11: full library restyle: every species re-rendered
  ...
  var TABLE_VERSION = 'r18'; // r18: dims.json + masks.json rebuilt for all 51 species. // r17: dims.json + masks.json rebuilt for 32 species.
  var ART_REVISIONS = {
    'aphelocoma-woodhouseii': 'anatomy-1',
    // Redrawn: the r16 perched pose had two claw fragments floating free of the foot.
    'caligavis-chrysops': 'feet-1'
  };
  ...
  function atlasStyle() {
    ...
    // Default flipped from upstream's 'stamps'. Those stamp designs draw the
    // species silhouette, and a species we have not illustrated has no
    // silhouette — so it came out as a grey blob, and more than half the 51
    // species detected here have no artwork yet. The classic cards fall back
    // to the empty-nest illustration instead, which reads as "not drawn yet"
    // rather than as a rendering fault. An explicit stored choice still wins.
    return readLS(ATLAS_STYLE_KEY, 'classic') === 'stamps' ? 'stamps' : 'classic';
  }
  ...
  // The menu-footer copy of the switcher (web/index.html). The admin one is
  // wired where the settings overlay is built; this is static markup, so it
  // needs its own listener. applyTheme persists and re-syncs every
  // [data-theme-seg], so both stay in step.
  var menuThemeSeg = document.getElementById('menuThemeSeg');
  if (menuThemeSeg) menuThemeSeg.addEventListener('click', function (ev) {
  ...
  // Default changed from upstream's 'life' (rarest first). That order leads
  // with the one-off visitors, which are exactly the species least likely to
  // have been illustrated — so the first screen was mostly empty nests. Most
  // heard first puts the drawn birds up front, and they are also the ones
  // anyone here will actually recognise from the garden.
  ...
  // The loading mark: a small bird that swaps for another every so often, so
  // the wait reads as "listening for birds" rather than a spinner. These are
  // ~128px thumbnails in web/loader/ (a few KB each), not the full cutouts —
  // the collage art is ~600KB a bird and would make the loader the slowest
  // thing on the page. Shuffled per render so it doesn't always open on the
  // same bird. The stagger lives in CSS (see .nest-listening .lb). Only pick
  // species whose illustration is already cut out (RGBA): an un-cut one
  // shrinks to a paper-coloured card on the dark page.
  var LISTENING_BIRDS = [
    'trichoglossus-moluccanus', 'gymnorhina-tibicen', 'eolophus-roseicapilla',
    'dacelo-novaeguineae', 'alisterus-scapularis', 'glossopsitta-concinna',
    'cacatua-galerita', 'zosterops-lateralis', 'anthochaera-carunculata',
    'manorina-melanocephala'
  ];
  function listeningBirdsMarkup() {
    var order = LISTENING_BIRDS.slice();
    for (var i = order.length - 1; i > 0; i--) {
      var j = Math.floor(Math.random() * (i + 1));
      var t = order[i]; order[i] = order[j]; order[j] = t;
    }
    return '<span class="listening-birds" aria-hidden="true">' + order.map(function (slug, k) {
      return '<img class="lb" style="--i:' + k + '" src="./loader/' + slug + '.webp" alt="" decoding="async">';
    }).join('') + '</span>';
  }
  ...
  function renderCollage(items, animate) {
    ...
    if (!items.length) {
      ...
      // Both captions ship; .is-fetching decides which is shown. The loading
      // one deliberately does NOT carry .empty, because the e-ink frame
      // overrides .empty's text and should never pick this one up.
      collage.innerHTML = '<div class="empty-nest">' +
        '<div class="nest-listening" role="status">' + listeningBirdsMarkup() +
        '<p>listening&hellip;</p></div>' +
  ...
  // A quiet "still fetching" mark on the collage.
  //
  // Two reasons it exists, and the second is the one that matters. The
  // obvious one: changing the window leaves the previous window's birds on
  // screen while the new ones are fetched, and nothing says so. The other is
  // that before the first response lands the collage renders the empty nest,
  // whose caption reads "no detections heard in this window" — which is not a
  // missing indicator but a false statement, because we do not know yet.
  // While this flag is up, that caption is replaced rather than decorated.
  //
  // (In practice the first load draws nothing until the data lands - the
  // collage is an empty box, not the nest.
  // So the cycling birds show in #collageListening, in the header band
  // under the title, which is what a window change and a slow first load actually see.
  // It is refilled each time so the birds come in a fresh order.)
  //
  // Delayed, because a warm cache settles in about 10ms and a mark that
  // appears and disappears inside a frame is worse than no mark at all.
  // Counted, because more than one fetch can be in flight at once.
  var COLLAGE_FETCH_GRACE_MS = 320;
  var collageFetchTimer = null;
  var collageFetchDepth = 0;
  function setCollageFetching(on) {
    collageFetchDepth = Math.max(0, collageFetchDepth + (on ? 1 : -1));
    if (collageFetchDepth > 0) {
      if (collageFetchTimer) return;
      collageFetchTimer = setTimeout(function () {
        collageFetchTimer = null;
        if (collageFetchDepth > 0) {
          var pill = document.getElementById('collageListening');
          if (pill) pill.innerHTML = listeningBirdsMarkup() + '<p>listening&hellip;</p>';
          collage.classList.add('is-fetching');
        }
      }, COLLAGE_FETCH_GRACE_MS);
      return;
    }
    clearTimeout(collageFetchTimer);
    collageFetchTimer = null;
    collage.classList.remove('is-fetching');
  }
  ...
  // The species card's own play button, beside Wikipedia: the same "best
  // recent clip" the atlas card plays (recording.php by name alone), so the
  // boys can hear the bird without opening the recordings list. It shares
  // the atlas card's button states and takes part in the one-audio-at-a-time
  // claim, so starting it stops any other sound and vice versa.
  var modalPlayAudio = null;
  function stopModalPlay() {
    audioRelease(stopModalPlay);
    var audio = modalPlayAudio;
    modalPlayAudio = null;
    if (audio) {
      try { audio.pause(); } catch (e) { }
    }
    setAtlasCardButtonState(document.getElementById('modalPlay'), 'idle');
  }
  function toggleModalPlay() {
    var btn = document.getElementById('modalPlay');
    if (!btn) return;
    if (modalPlayAudio) { stopModalPlay(); return; }
    if (!activePostcardSci) return;
    audioClaim(stopModalPlay);
    setAtlasCardButtonState(btn, 'loading');
    var audio = new Audio(mediaApiUrl('recording', { sci: activePostcardSci }, activePostcardEducatorScope));
    modalPlayAudio = audio;
    audio.addEventListener('canplay', function () {
      if (modalPlayAudio !== audio) return;
      setAtlasCardButtonState(btn, 'playing');
      audio.play();
    });
    audio.addEventListener('ended', function () {
      if (modalPlayAudio === audio) stopModalPlay();
    });
    audio.addEventListener('error', function () {
      if (modalPlayAudio !== audio) return;
      audioRelease(stopModalPlay);
  ...
  (function () {
    var btn = document.getElementById('modalPlay');
    if (btn) btn.addEventListener('click', toggleModalPlay);
  })();
  ...
  function refreshRecent(animate) {
    ...
    var forHours = educatorScopeId() ? 1000000 : currentHours;
    var request = educatorScopeRequest();
    // Only on a deliberate refetch. The 30-second poll passes no animate, and
    // a mark that blinks every half minute is noise, not feedback.
    if (animate) setCollageFetching(true);
    return scopedFetchJson('recent', { hours: forHours }, request)
      ...
      .then(function () { if (animate) setCollageFetching(false); });
  }
  function refreshAll(animate) {
    ...
    // The collage reads DATA.recent.species and nothing else, so draw it the
    // moment that one response lands instead of waiting on the other seven.
    // Promise.all finishes with its SLOWEST member, and the slow ones here are
    // the historical aggregates: measured cold, recent came back in 5.0s while
    // firstseen took 24.4s, so the first bird appeared at 25.6s on a screen
    // that had everything it needed at 5. Same single request either way -
    // this adds no fetch and no cache write, it just stops discarding an early
    // answer. The full handler below re-renders once the rest arrives.
    var recentFetch = tolerate(scopedFetchJson('recent', { hours: forHours }, request));
    // Cleared when the collage's own data lands, not when all eight do. The
    // historical aggregates keep loading behind the birds and the collage is
    // not waiting on them — see the early render below.
    if (animate) {
      setCollageFetching(true);
      var settled = function () { setCollageFetching(false); };
      recentFetch.then(settled, settled);
    }
    if (!transactional) recentFetch.then(function (j) {
      // Same staleness guards as the handler below: a window or scope change
      // mid-flight must not repaint the collage with the wrong window.
      if (!j || request.generation !== educatorScopeGeneration
        || forHours !== (educatorScopeId() ? 1000000 : currentHours)) return;
      DATA.recent = j;
      if (liveStats) DATA.statsRecent = j;
      if (typeof j.site_name === 'string') applySiteName(j.site_name);
      renderCollageFromData(animate);
    });
    return Promise.all([
  ...
  function populatePostcard(
    ...
    if (needsArt) {
      ...
      imageReady = postcardPoseAvailability(sci, poseBtns).then(function (results) {
        ...
        // Default to the LOWEST-numbered available pose, i.e. perched: it is
        // how you actually see these birds, and it is the pose the collage
        // shows most of the time (FLY_PROB is 0.15). Upstream defaulted to
        // in-flight. A species the reader has explicitly toggled keeps their
        // choice. No readable pose gets a deterministic nest fallback.
        ...
        }) || available.slice().sort(function (a, b) { return a.pose - b.pose; })[0];
        if (!pick) {
          poseToggle.setAttribute('data-unavailable', 'true');
          if (artwork) artwork.setAttribute('data-art-state', 'fallback');
          img.src = './nest-eggs.webp';
